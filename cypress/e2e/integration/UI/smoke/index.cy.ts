...
describe("Smoke Test Suite", () => {
  beforeEach(() => {
    ...
    cy.intercept("**/metadata/platforms/salesforce/objects/Opportunity/**").as(
    ...
    cy.loginByCognitoApi(
      Cypress.env("TEST_AUTOMATION_USERNAME"),
      Cypress.env("TEST_AUTOMATION_PASSWORD")
    );
  });
  ...
  it("should perform general user sanity checks", () => {
    ...
    journeys.journeyGoalsModal.JourneyNameInput.clear().type(
      AUTOMATIN_JOURNEY_NAME
    );
    ...
    journeyBuilder.updateOpportunity.recordCriteriaLocator.dragAndDropField(
      "Name",
      0
    );
    ...
    journeyBuilder.updateOpportunity.recordCriteriaLocator.ItemContainer.type(
      "Competitor"
    );
    ...
    journeyBuilder.updateOpportunity.datePicker.DatePickerPanel.first()
      .contains("Today")
      .click();
    journeyBuilder.updateOpportunity.select.Select.first().click();
    journeyBuilder.updateOpportunity.select.SelectOption.contains(
      "Prospecting"
    ).click({ force: true });

    cy.get('[title="checkbox_fieldname"]').scrollIntoView();
    journeyBuilder.updateOpportunity.select.Select.eq(6).click();
    journeyBuilder.updateOpportunity.select.SelectOption.contains("True").click(
      { force: true }
    );
