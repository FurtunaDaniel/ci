...
class JourneyBuilder {
  ...
  dropRuleNode(
    rule: "ifElse" | "timeDelay" | "loop" | "merge" | "waitForTrigger",
    position?: number
  ) {
    cy.dragAndDrop({
    ...
    Object.keys(this.segment.STRING_OPERATORS).forEach((operator) => {
      ...
      if (this.isMultiValue(stringOperator)) {
        ...
        this.segment.queryBuilder.multiValueInput.MultiValue.last()
          .click()
          .type(
            `${this.segment.STRING_1} ${stringOperator}{enter}${this.segment.STRING_2} ${stringOperator}{enter}`
          );
      } else {
        this.segment.queryBuilder.ItemContainer.last()
          .type(
            `${this.segment.STRING_OPERATORS[operator]} ${this.segment.STRING_1}`
          )
          .trigger("focusout");
      }
    });
  ...
  addSelectOperatorsFields(fieldName: string) {
    ...
    Object.keys(this.segment.SELECT_OPERATORS).forEach(
      (operator, operatorIndex) => {
        ...
        cy.get(this.segment.queryBuilder.select.selectSearchInput)
          .last()
          .click();
  ...
  addNumericOperatorsFields(fieldName: string) {
    ...
    Object.keys(this.segment.NUMBER_OPERATORS).forEach(
      (operator, operatorIndex) => {
        this.segment.dragAndDropField(fieldName, 0);
        ...
        switch (currentOperator) {
          ...
          case this.segment.NUMBER_OPERATORS.IsNotOneOf:
            this.segment.queryBuilder.multiValueInput.MultiValue.last()
              .click()
              .type(
                `${this.segment.NUMBER_1}{enter}${this.segment.NUMBER_2}{enter}`
              );
            break;
  ...
  addDateOperatorsFields(fieldName: string) {
    ...
    Object.keys(this.segment.DATE_OPERATORS).forEach(
      (operator, operatorIndex) => {
        this.segment.dragAndDropField(fieldName, 0);
        ...
        switch (currentOperator) {
          ...
          default:
            ...
            this.segment.queryBuilder.datePicker.DatePickerPanel.last()
              .contains("Today")
              .click();
            break;
        }
  ...
  checkStringOperatorsFields(fieldName: string) {
    ...
    Object.keys(this.segment.STRING_OPERATORS).forEach(
      (operator, operatorIndex) => {
        const currentOperator = this.segment.STRING_OPERATORS[operator];

        this.segment.queryBuilder.GroupContainer.find(
          this.segment.queryBuilder.criteriaBuilderItemTitle
        )
        ...
        cy.get("@currentGroup")
          .find(this.segment.queryBuilder.select.operatorSelect)
          .contains(currentOperator);
    ...
    Object.keys(this.segment.SELECT_OPERATORS).forEach(
      (operator, operatorsIndex) => {
        ...
        cy.get("@currentGroup")
          ...
          .each(($selectItem) => {
            cy.wrap($selectItem).contains(
              `${this.segment.sideBar.deliveryStatuses[statusIndex++]}`
            );
          });
      }
    );
        ...
        switch (currentOperator) {
          ...
          default:
            cy.get("@currentGroup")
              ...
              .should(
                "have.value",
                `${this.segment.NUMERIC_PREFIX_VALUE}${operatorsIndex}`
              );
  ...
  isMultiValue(operator) {
    const multiValueOperators = [
      this.segment.STRING_OPERATORS.IsOneOf,
      this.segment.STRING_OPERATORS.IsNotOneOf,
    ];
  ...
  addCriteria({ platform, object, field, groupIndex = 0 }: AddCriteriaProps) {
    ...
    this.segment.queryBuilder.GroupContainer.eq(groupIndex).trigger("dragend", {
      force: true,
    });
  }
  ...
  checkOperatorAndValue({ operator, value1, value2 }: CheckOperatorAndValue) {
    this.segment.queryBuilder.select.OperatorSelect.click().then(() => {
      this.segment.queryBuilder.select.OperatorPopupContainer.contains(
        operator
      ).click({ force: true });
      this.segment.queryBuilder.multiValueInput.MultiValue.click().type(
        `${value1}{enter}${value2}{enter}`
      );
      ...
      this.segment.queryBuilder.multiValueInput.Tag.eq(0).should(
        "have.text",
        value1
      );
      this.segment.queryBuilder.multiValueInput.Tag.eq(1).should(
        "have.text",
        value2
      );
      ...
      this.segment.queryBuilder.multiValueInput.Tag.contains(value2).should(
        "not.exist"
      );
      this.segment.queryBuilder.multiValueInput.MultiValue.click().type(
        `${value2}{enter}`
      ...
      this.segment.queryBuilder.multiValueInput.MultiValuePopup.contains(
        value2
      ).click();
